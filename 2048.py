...
class Play_2048(Tk):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.canvas = Canvas(self, width=410, height=410, borderwidth=5, highlightthickness=0, background="#444")
    ...
    def new_tiles(self):
        ...
        while self.full() == False:
            ...
            if (self.game_board[x][y] == 0):
                ...
                if num == 2:
                    self.square[x, y] = self.canvas.create_rectangle(x1, y1, x2, y2, fill="#eee4da", tags="rect",outline="", width=0)                                                 
                    self.canvas.create_text((x1 + x2) / 2, (y1 + y2) / 2, font=("Arial", 36), fill="#776e65", text="2")
                elif num == 4:
                    self.square[x, y] = self.canvas.create_rectangle(x1, y1, x2, y2, fill="#ede0c8", tags="rect",outline="", width=0)                                                 
                    self.canvas.create_text((x1 + x2) / 2, (y1 + y2) / 2, font=("Arial", 36), fill="#776e65", text="4")
                break
    ...
    def show_number0(self, row, column, a, b, c, d):
        self.square[row, column] = self.canvas.create_rectangle(a, b, c, d, fill="#cdc1b4", tags="rect", outline="")

    def show_number(self, row, column, a, b, c, d, num):
        bg_color = {'2': '#eee4da', '4': '#ede0c8', '8': '#f2b179', '16': '#f59563', '32': '#f67c5f', '64': '#f65e3b', '128': '#edcf72',
                    '256': '#edcc61', '512': '#edc850', '1024': '#edc53f', '2048': '#edc22e'}
        color = '#776e65'
        block_sizes = {'128': 30, '256': 30, '512': 30, '1024': 20, '2048': 20}

        self.square[row, column] = self.canvas.create_rectangle(a, b, c, d, fill=bg_color[str(num)], tags="rect", outline="")
        self.canvas.create_text((a + c) / 2, (b + d) / 2, font=("Arial", block_sizes.get(str(num), 36)), fill=color, text=str(num))
